...
class SubscribeController {
  static async subscribeUser(request: Request, response: Response) {
    try {
      ...
      if (
        subscribeRequest.termsAndConditions !== subscribeRequest.participate
      ) {
        return RequestHelper.sendError(response, 400, {
      ...
      return RequestHelper.sendSuccess(response, {
        message: "User subscribed",
      });
    } catch (err) {
      console.error(err);

      return RequestHelper.sendError(response, 500, {
        errors: { message: "Unable to subscribe user" },
      });
    }
  }
}
